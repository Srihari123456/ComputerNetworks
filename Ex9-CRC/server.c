...
char* generateCRC(char *s, char *b) {
 ...
 t = 0;
 for(i = strlen(a) - strlen(b)+1; i < strlen(a); i++)
 a[t++] = a[i];
 a[t] = '\0';
 return a;
}
int main(){
 ...
 buff[val] = '\0';
 strcpy(data, buff);
 printf("In Server, the data received from the client is: %s\n",buff);
 strcpy(div, generateCRC(data, div));
 printf("The remainder we get is:%s\n", div);
 for(i = 0; i < strlen(div); i++) {
 if(div[i] != '0') {
 printf("Oops Incorrect data obtained\n");
 flag = 1;
 break;
 }
 }
 if(flag == 0)
 printf("Correct data obtained\n");
 return 0;
}
